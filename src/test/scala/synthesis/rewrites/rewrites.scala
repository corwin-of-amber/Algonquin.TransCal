...
package object rewrites {
  def VersionedHyperGraphGenFactory[Node, Edge](edgeSource: Gen[HyperEdge[Node, Edge]]): Gen[VersionedHyperGraph[Node, Edge]] =
    edgeSource.map(VersionedHyperGraph(_))
  ...
  val hyperGraphGen: Gen[HyperGraph] = VersionedHyperGraphGenFactory(hyperGraphEdgeGen)
