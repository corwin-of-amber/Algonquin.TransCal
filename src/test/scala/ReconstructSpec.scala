...
//    val root: Identifier = new Identifier(arr.head.toString)
//    val subtrees = for (obj <- arr.tail) yield {
//      obj match {
//        case JInt(x) => new Tree(new Identifier(x.toString))
//        case l: JArray => jarrayToTree(l.values, encoding)
//        case a: List[JValue] => jarrayToTree(a, encoding)
//        case num: BigInt => new Tree(new Identifier(num.toString))
...
//      val words: Seq[OriginalEdge[Int]] = (for(f <- finals.toSeq) yield {encoding --> f}).append(
//        test drop 4 map ((line: String) => line split " " map (_.toInt)) map (OriginalEdge(_))
//      )
//      for (e <- except) {
//        encoding --> e
//      }
////      for (w <- words; n <- w) {
////        encoding.ntor --> new Identifier(n)
////      }
//
//      val outs: List[Term] = treeArrayToTrees(parse(test(3)).as[JArray], encoding) toList
//      val newOuts: List[Term] = {
//        val reconstruct = new Reconstruct(init, words)
