...
/** An algebric datatype definition.
  ...
  */
case class Datatype(name: Identifier, typeParameters: Seq[Identifier], constructors: Seq[Identifier]) {
...
//  }))
  ...
  def asType: AnnotatedTree = AnnotatedTree.withoutAnnotations(name, typeParameters.map(AnnotatedTree.identifierOnly))
}
