...
object TupleTimeComplexRewrite extends VersionedOperator[RewriteSearchState]{
  ...
  private val tupleTimeComplexEdge = HyperEdge(ReferenceTerm(0), ExplicitTerm(HyperTermIdentifier(Language.tupleId)), Seq(RepetitionTerm.rep1(Int.MaxValue, Stream.from(1).map(ReferenceTerm(_))).get), EmptyMetadata)
  ...
  override def apply(state: RewriteSearchState, lastVersion: Long): (RewriteSearchState, Long) = {
    ...
    val newMatchTimeComplexEdges = state.graph.findRegexHyperEdges(tupleTimeComplexEdge).flatMap(
      edge => {
        ...
        println("sourcesTimeComplexes - " + sourcesTimeComplexes.count(_.hasNext) + " size - " + sourcesTimeComplexes.size)
