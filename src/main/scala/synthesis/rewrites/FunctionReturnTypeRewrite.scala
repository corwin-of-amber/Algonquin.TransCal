...
object FunctionReturnTypeRewrite extends VersionedOperator[RewriteSearchState] {
  ...
  private val functionReturnTypeEdge = patternEdgeCreator(functionTypeIdHole, Language.mapTypeId, Seq(Ignored(), Ignored(), functionReturnTypeHole))
  private val functionApplicationEdge = patternEdgeCreator(functionApplicationHole, functionIdentifierHole, Seq(Ignored(), Ignored()))
