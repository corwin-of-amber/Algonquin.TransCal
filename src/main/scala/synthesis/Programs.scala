  ...
  private def destruct(tree: Term): RewriteSearchState.HyperGraph = {
    ...
    def destruct(tree: Term, counter: () => Int): Set[HyperEdge[HyperTerm, HyperTermIdentifier]] = {
      if (tree.isLeaf) {
        ...
        val newHyperEdge = HyperEdge[HyperTerm, HyperTermIdentifier](HyperTermId(counter()), HyperTermIdentifier(Identifier(tree.root)), tree.subtrees.map(_.root).map(HyperTermIdentifier))
