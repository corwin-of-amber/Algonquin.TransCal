...
object CoqAst {
  ...
  def parseId(arr: JsArray) = {
    assert((arr \ 0).validate[JsString].get.value == "Id")
    (arr \ 1).validate[JsString].get.value
  }
  ...
  def parseModPath(jsArray: JsArray) = {
    (jsArray \ 0).validate[JsString].get.value match {
      case "MPfile" =>
        ...
        mpArr.value.tail.map(js => parseId(js.validate[JsArray].get)).mkString(".")
      case "MPbound" =>
  ...
  def fromJson(json: JsArray, types: Seq[CoqAst]): CoqAst = {
    val termLiteral = (json \ 0).validate[JsString].get.value
    termLiteral match {
      case "Lambda" | "Prod" =>
        assert((json \ 1 \ "binder_name" \ 1 \ 0).validate[JsString].map(_.value).contains("Id"))
      ...
      case "App" =>
        ...
        val paramTerms = json.value.map(js => fromJson((js \ 2).validate[JsArray].get, types))
      ...
      case "Rel" =>
        ...
        types((json \ 1).validate[JsNumber].get.value.toInt)
      case "Const" =>
        val const = (json \ 1)
      ...
      case "Construct" =>
        throw new NotImplementedException("Implement once encountered a construct example")
      case "LetIn" =>
