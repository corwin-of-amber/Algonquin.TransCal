...
\begin{proposal}E-graphs are a suitable formalism to serve as the bridge that unifies equality reasoning in SMT (Nelson-Oppen) and ATP (superposition/paramodulation).
\end{proposal}

We propose to build a framework for equality reasoning based on e-graphs and term rewriting systems (TRSs), 
inspired by similar uses that came up in compiler optimizations and program synthesis~\cite{POPL2009:Tate,PLDI2015:Wilcox,FM2018:Becker,PLDI2020:Nandi}.
We will use \emph{egg}~\cite{POPL2021:Willsey}, a collection of state-of-the-art algorithms for manipulating e-graphs and for fast rewriting over the e-graph representation.
...
Previous work in compilers relied on the concept of \emph{equality saturation}~\cite{arXiv2020:Singher}:
