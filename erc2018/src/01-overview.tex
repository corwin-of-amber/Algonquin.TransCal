...
for specifications~\cite{LPAR2010:Leino,ECOOP2013:Regis,ICSE2009:Schulte,PASTE2005:Barnett,CAV2015:Gurfinkel}, loop invariants~\cite{VMCAI2011:Bradley,CAV2013:Komuravelli,CAV2019:Feldman}, rely-guarantee properties for concurrent programs~\cite{relyguarantee1,TCS2007:Jones}, and for defining the semantics of the programs themselves~\cite{MACS1967:Floyd}.
...
both of whom developed calculi for algorithmic formalization of proofs.
As computers became available, it was natural that these notions be implemented as programs.
As early as 1960~\cite{Wiley1960:Wang,Wiley1961:Wang}, large automatic systems for theorem proving have been used by the research community for a variety of logical reasoning tasks.
...
To support efficient rewriting and avoid backtracking, modern ATPs such as Vampire and E employ an order relation on terms, so that all rewrites are \emph{directed}.
...
I propose, as a unifying mechanism, to further utilize the notion of \emph{equality graphs} (e-graphs), as is done in some SMT solvers~\cite{IC2007:Nieuwenhuis,CADE2007:deMoura,TACAS2017:Barbosa} for performance purposes.
I combine this with term rewriting and \emph{equality saturation}~\cite{POPL2009:Tate} to obtain \emph{unordered} sets of equivalence terms (e-classes).
...
I will focus on the incorporation of Separation Logic and its modern variants such as Iris~\cite{POPL2015:Jung,ICFP2016:Jung,ESOP2017:Krebbers} for modeling a program's memory, \esp in programs that use dynamic allocation or multiple threads with shared state.
...
The inclination of SMT solvers to integrate concepts of induction~\cite{VMCAI2015:Reynolds} and higher-order reasoning~\cite{CADE2019:Barbosa} on one hand,
...
The Matryoshka project (\cite{CADE2019:Barbosa,Misc2020:Tourret,CADE2020:Vukmirovic} and more) set as their target to enrich SMT solvers with higher-order constructs such as recursive definitions and second-order quantification.
...
\medskip
The grand pursuit of this project will be to bring automated reasoning into large undertakings in formal methods such as the DeepSpec expedition~\cite{OOPSLA2016:Pierce}.
This will have enormous impact as it will save many man-years in future endeavors and will catalyze the readiness of automated verification tools for industrial use.